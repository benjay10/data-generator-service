...
export function removeBookPattern(bookUri, relation, sudo) {
  const bookGraph = (sudo ? mu.sparqlEscapeUri(conf.DEFAULT_GRAPH) : "?bookf");
  const fileGraph = (sudo ? mu.sparqlEscapeUri(conf.DEFAULT_GRAPH) : "?bookg");
  ...
  switch (relation) {
    ...
    case "withfile":
      filepart = `
        GRAPH ${bookGraph} {
          OPTIONAL { ${bookUri} sch:associatedMedia ?vuri . }
        }
        GRAPH ${fileGraph} {
          OPTIONAL {
            ?vuri ?q ?r .
