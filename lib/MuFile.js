...
export function removeFilePattern(fileUri, relation, sudo) {
  const fileGraph = (sudo ? mu.sparqlEscapeUri(conf.DEFAULT_GRAPH) : "?fileg");
  const bookGraph = (sudo ? authorGraph : "?filef");
  ...
  switch (relation) {
    ...
    case "withreferences":
      refpart = `
        GRAPH ${bookGraph} {
          ?book sch:associatedMedia ${fileUri} .
        }`;
      break;
  }
