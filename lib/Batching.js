...
async function processBatch() {
  ...
  const { batchMuUuid, batchNum, batches, itemsPerBatch, pausePerBatch, targetGraph, useResources, withFiles, sudo, fileSize, authorUri } = batch;
  ...
  if (useResources) {
    await res.saveBooks(books);
  }
  else {
    //Save a physical copy of the books
    books.forEach(b => b.saveFile());
