...
async function getBooks(type, id) {
  console.log("Getting books via url:", `${conf.RESOURCE_HOST}/${type}/${id}/books`);
  const request = await fetch(`${conf.RESOURCE_HOST}/${type}/${id}/books`, {
    method: "GET"
  });
  if (! request.ok)
...
export async function deleteAuthor(authorUuid, relation) {
  ...
  switch (relation) {
    ...
    case "withbooks":
      //Get all book id's
      bookIDs = (await getBooks("authors", authorUuid)).data.map(b => b.id);
      break;
  }
